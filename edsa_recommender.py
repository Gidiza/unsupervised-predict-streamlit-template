...
with open('movies.jpg', "rb") as image_file:
...
def main():
	...
	with st.sidebar:
		...
		icons=['house', 'pie-chart', 'star-fill','people-fill', 'info-square-fill','phone'], menu_icon="cast", default_index=0)
	...
	if selection == "Home":
		# Header contents
		st.image('logo teal.png', width=600)
		...
		movie_2 = st.selectbox('Second Option',title_list[100:999])
		movie_3 = st.selectbox('Third Option',title_list[2000:2457])
	...
	# ------------- SAFE FOR ALTERING/EXTENSION -------------------
		...
		st.markdown("<h2 style=color:#3FBEBF;>Visualisation</h2>",unsafe_allow_html=True)
		...
		with col2:
			...
			st.write("Fabian has worked as a Project Manager, Product Manager, Systems and Production developer. When he is not coding he enjoys watching sport on television.")
		with col1:
			st.image('fabian.png', width=200)
		col1, mid, col2 = st.columns([80,10,80])
		with col1:
			st.subheader("Angela Morris - Dep Director")
			st.write("Angela has worked as a Product Manger and has worked with many startups. On her spare time she enjoys reading novels.")
		with col2:
			st.image('angela.jpg', width=200)
		col1, mid, col2 = st.columns([80,10,80])
		with col2:
			st.subheader("Malesela M. Mohlake - Senior Data Analyst")
			st.write("Mr Mohlake has worked as a lead data analyst for multiple F.A.N.G. companies. When he's not working he enjoys spending time with his family and friends.")
		with col1:
			st.image('marcus.jpg', width=200)
		col1, mid, col2 = st.columns([80,10,80])
		with col1:
			st.subheader("Lungelo Ndlovu - Senior Data Analyst")
			st.write("Lungelo has worked as researcher for MIT in statistics. Lungelo enjoys turtoring young aspiring data sciensist and data analyst alike.")
		with col2:
		...
		with col1:
			st.image('lindokuhle.jpeg', width=200)
		...
		col1,mid,col2 = st.columns([80,10,80])
		with col1:
			st.image('logo teal.png',width=400)
		with col2:
		...
		#	st.write(rat[True exec("act_s and war_s and rom_s and com_s and drm_s and adv_s and sf_s and thr_s and ani_s")])
		#st.bar_chart(xy)
